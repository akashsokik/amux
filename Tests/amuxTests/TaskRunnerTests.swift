...
struct TaskRunnerTests {
    ...
    func echo() async throws {
        ...
        let wt = NSTemporaryDirectory()
        let task = RunnerTask(id: "t", name: "t", command: "echo hi",
                              cwd: nil, source: .pinned, isOverridden: false)
        runner.start(task, worktreePath: wt)
        try await Task.sleep(nanoseconds: 1_500_000_000)
        let session = runner.session(for: task.id, worktreePath: wt)
        #expect(session != nil)
    ...
    func stop() async throws {
        ...
        runner.stop(id: task.id, worktreePath: wt)
    ...
    func restart() async throws {
        ...
        runner.start(task, worktreePath: wt)
        try await Task.sleep(nanoseconds: 300_000_000)
        let firstPid = runner.session(for: task.id, worktreePath: wt)?.pid
        runner.restart(task, worktreePath: wt)
        try await Task.sleep(nanoseconds: 4_000_000_000)
        let secondPid = runner.session(for: task.id, worktreePath: wt)?.pid
